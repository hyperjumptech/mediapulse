/**
 * Parsed data source string: db:table:field with optional query options.
...
const DATA_SOURCE_REGEX =
  /^([a-z]+):([a-zA-Z][a-zA-Z0-9]*):([a-zA-Z_][a-zA-Z0-9_]*)(?:\?(.*))?$/;

/**
 * Returns true if the value looks like a data source string (db:table:field).
 *
...
export const parseDataSourceString = (str: string): DataSourceParsed | null => {
  ...
  const [, source, table, field, query] = match;
