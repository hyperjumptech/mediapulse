...
import { logger as defaultLogger, slimPinoLogger } from "@workspace/logger";
 ...
 *
 * When **hermes-worker** invokes the agent, it may send `X-Job-Id`, `X-Execution-Id`, `X-Schedule-Id`,
 * `X-Schedule-Execution-Id`, and `X-Pipeline-Step-Id`; those are passed to `run` on
 * `context.hermesCorrelation` when present.
 *
