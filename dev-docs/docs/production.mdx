...
| `AGENT_API_KEY`        | Depends on app | **Required** for hermes and hermes-worker when triggering pipelines or calling agent-data-api. Create via `apps/hermes/scripts/generate-api-key.ts <email> "<name>"` (or insert into the API keys table). Store securely; never expose to the client; the raw key is printed once.                                    |
