...
| **DOMAIN_INTEGRATION_BEARER_API_KEY** | No       | Optional. Dashboard API key (purpose `domain_integration`) for JWTs to domain integration HTTP. If omitted, **`HERMES_INTERNAL_API_KEY`** mints JWTs via **`POST /api/token`**. The **base URL** comes from the active domain integration row in orchestration. |
