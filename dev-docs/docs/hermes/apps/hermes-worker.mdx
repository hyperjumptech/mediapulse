...
| **DOMAIN_INTEGRATION_AUTH_TOKEN** | No       | Bearer for domain-api when expanding step inputs; must match domain-api. The **base URL** comes from the active domain integration row in orchestration (registration), not from env. |
