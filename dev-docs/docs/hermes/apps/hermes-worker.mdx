...
- **Cancellation-aware completion** — `invoke_agent` completion updates now handle schedule, HTTP trigger, and manual executions. If an execution was already cancelled, completion does not overwrite the terminal `cancelled` run status.
