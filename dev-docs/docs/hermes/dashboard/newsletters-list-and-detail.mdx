...
The list and detail pages use Hermes' generic `table-v1` template (`DomainTablePage`) against the manifest and `detailBlocks` from domain-api. The newsletters manifest opts into list filters and default sort; filter UI is shared with other table-v1 pages via manifest `listFilters` / `defaultSort`.
...
| Column      | Notes                                                                                                                  |
| ----------- | ---------------------------------------------------------------------------------------------------------------------- |
| Ticker      | Symbol from the joined `Ticker`. Filter via ticker dropdown (`?tickerId=`).                                          |
| Subject     | Searchable substring filter (`?q=`).                                                                                   |
| Created     | Sortable column header (`?sort=createdAt&dir=`); defaults to **newest first** (`createdAt` desc). Filter with **From date** / **To date** (`?from=` / `?to=`, `YYYY-MM-DD`; single day, range, or open-ended). |
...
The ticker dropdown is populated from `GET .../meta` (`tickerOptions: { value, label }[]`). Date inputs use the reusable `CreatedDateRangeFilter` component (`apps/hermes/dashboard/components/created-date-range-filter.tsx`) for other table-v1 pages that declare `listFilters: ["createdAt"]`.
