...
This guide walks through configuring Azure, PowerShell, and Exchange Online so that `@workspace/outlook-inbox` can access shared mailboxes via the Microsoft Graph API with application permissions.
...
After completing Azure and Exchange Online setup, use the values (Tenant ID, Client ID, Client Secret, shared mailbox address) in your app or in `@workspace/outlook-inbox` client configuration. See the [outlook-inbox](/packages/outlook-inbox) package doc for usage.
