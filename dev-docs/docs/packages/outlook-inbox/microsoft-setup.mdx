...
After completing Azure and Exchange Online setup, use the values (Tenant ID, Client ID, Client Secret, shared mailbox address) in your app or in `@mediapulse/outlook-inbox` client configuration. See the [outlook-inbox](/packages/outlook-inbox) package doc for usage.
...
**Setup Complete!** Your application can now access the specified shared mailboxes with no user interaction required. See the [outlook-inbox](/packages/outlook-inbox) package for API usage.
