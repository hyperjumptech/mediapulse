---
title: Microsoft Graph setup
description: Complete setup guide for Microsoft Graph API with application permissions (Azure, PowerShell, Exchange Online)
icon: Settings
---

This guide walks through configuring Azure, PowerShell, and Exchange Online so that `@mediapulse/outlook-inbox` can access shared mailboxes via the Microsoft Graph API with application permissions.
...
After completing Azure and Exchange Online setup, use the values (Tenant ID, Client ID, Client Secret, shared mailbox address) in your app or in `@mediapulse/outlook-inbox` client configuration. See the [outlook-inbox](/mediapulse/packages/outlook-inbox) package doc for usage.
...
**Setup Complete!** Your application can now access the specified shared mailboxes with no user interaction required. See the [outlook-inbox](/mediapulse/packages/outlook-inbox) package for API usage.
