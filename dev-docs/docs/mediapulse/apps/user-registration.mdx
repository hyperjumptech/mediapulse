...
1. The `@mediapulse/user-registration` home page loads the ticker list from **agent-data-api** (`GET /api/v1/user-registration-tickers`, no bearer token—same as the unsubscribe routes) so choices stay aligned with the Mediapulse `ticker` table.
