...
| `intentWeights`                                           | No       | Per-intent merge and LLM target-count weights (default favors `breaking`, `kg_change`, and industry-wide intents such as `competitor`, `macro`, `regulatory`, and `industry_trend`; legacy `fundamental` weight is `0`). Keys: `breaking`, `kg_change`, `fundamental`, `sentiment`, `competitor`, `supply_chain`, `esg`, `macro`, `technical`, `regulatory`, `technology_trend`, `geopolitical`, `industry_trend`. |
| `templatePack`                                            | No       | Deterministic template pack name (default `default-v1`). Set to `rich-v2` for the expanded industry-focused pack, or `rich-v2-extended` for all twelve standard intents. Operators can switch packs via Hermes invoke config without redeploying the agent. |
...
| `competitor`        | Peer positioning, share shifts, competitive threats  | `Software competitive landscape`      | `0.7`          |
| `supply_chain`      | Suppliers, logistics, input costs, bottlenecks       | `Acme Co supplier risk`               | `0.4`          |
| `esg`               | Environmental, social, governance controversies    | `Acme Co ESG controversies`           | `0.5`          |
| `macro`             | Rates, FX, commodity, sector-wide drivers            | `Technology macro outlook 2026`       | `0.7`          |
| `technical`         | Chart patterns, momentum, support/resistance         | `ACME chart pattern May`              | `0.3`          |
| `regulatory`        | Licensing, compliance, policy enforcement            | `Software regulatory changes`         | `0.6`          |
| `technology_trend`  | Digital disruption, AI adoption, tech shifts         | `Software technology disruption`      | `0.55`         |
| `geopolitical`      | Trade, sanctions, cross-border sector dynamics       | `Software trade policy impact`        | `0.5`          |
| `industry_trend`    | Sector outlook, analyst views on the industry        | `Technology industry outlook 2026`    | `0.6`          |
| `fundamental`       | Legacy earnings/guidance intent (retained for old rows) | _(not emitted in new runs)_         | `0`            |

New runs prioritize external industry intelligence. The standard pipeline no longer emits `fundamental` queries.
...
| --------------------- | --------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------ |
| **`default-v1`**      | 5         | `{symbol}`, `{name}`, `{sector}`, `{industry}`                                                              | `Technology industry trends`   |
| **`rich-v2`**         | ~25       | `{symbol}`, `{name}`, `{topEntity}`, `{recentTheme}`, `{sector}`, `{industry}`, `{currentQuarter}`, `{currentYear}`, `{currentMonth}` | `Software competitive landscape` |
| **`rich-v2-extended`**| ~28       | Same as `rich-v2` plus templates for all twelve standard intents including `regulatory`, `technology_trend`, `geopolitical`, and `industry_trend` | `Software regulatory changes`  |
...
- **`{sector}`** — sector label from ticker metadata (`Sektor` / `sector`)
- **`{industry}`** — industry label from ticker metadata (`Industri` / `industry`)
