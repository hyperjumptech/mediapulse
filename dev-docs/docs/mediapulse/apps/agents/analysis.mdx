...
- **`defaultMaxBatchSize`** — When Hermes run input omits **`maxBatchSize`**, caps how many **`data_source`** rows are fetched and processed in one run (and sets **`analysis.get`** `limit`, capped by **`ANALYSIS_GET_DATA_SOURCE_LIMIT_MAX`** on the API, currently **10**). Set this on the **article-analysis** agent config JSON in Hermes to tune per-run work below that ceiling; the package default is **10** when the key is omitted.
...
Tuning for extraction, chunking, **`scoreBreakdownVersion`** (maps to `_version` in stored breakdowns), relevance weights, selection caps, **`runPolicy`**, POST retries, **`defaultMaxBatchSize`** (per-run source cap when input omits `maxBatchSize`; default **10** in code), and **debounce** fields lives in the **article-analysis** agent config in Hermes (Zod schema `articleAnalysisConfigSchema`). **`openaiApiKey`** and model limits are set there—**not** via `OPENAI_*` process env on the agent container.
