...
| `fundamental`    | Earnings, filings, guidance-style fundamentals (de-prioritized; low default weight) |
...
Optional additional numeric keys may appear for experiments. **Weighted `score`** is in `[0, 1]` and defaults to tuned weights over the five dimensions (`breakingNews` `0.2`, `kgRelation` `0.3`, `fundamental` `0.05`, `tickerSalience` `0.2`, `sourceQuality` `0.25`); tune via Hermes config (`relevanceWeight*` keys).
