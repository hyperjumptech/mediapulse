...
- **GET** — Query (`getAnalysisQuerySchema`):
  ...
  - `limit` — optional positive integer (max **10**, see **`ANALYSIS_GET_DATA_SOURCE_LIMIT_MAX`** in the contract). When set, `dataSources` returns at most `limit` rows (ordered by `createdAt` ascending) while **`dataSourceTotalCount`** still reflects the full filtered count.
