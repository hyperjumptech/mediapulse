...
- **GET** — Query (`getAnalysisQuerySchema`):
  ...
  - `limit` — optional positive integer, **at most 10** per request (hard cap in `getAnalysisQuerySchema`). Article-analysis sets `limit` from Hermes config via `min(maxBatchSize, analysisGetDataSourceLimitMax)` (defaults **10**). When set, `dataSources` returns at most `limit` rows (ordered by `createdAt` ascending) while **`dataSourceTotalCount`** still reflects the full filtered count.
