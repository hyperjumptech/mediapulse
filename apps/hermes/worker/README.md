...
- **Env:** `PG_DATAQUEUE_DATABASE` and `ORCHESTRATION_DATABASE_URL` required; **HERMES_INTERNAL_API_KEY** (same preset as `packages/hermes/env`) and **AGENT_AUTH_API_URL** for JWT-based schedule execution. Set **REQUIRE_HTTPS_AGENT_ENDPOINTS=true** in production to enforce HTTPS for agent endpoints (localhost is always allowed).
