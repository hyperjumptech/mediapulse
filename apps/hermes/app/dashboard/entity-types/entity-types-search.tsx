...
/**
 * Search form for entity types by name. Submits via GET to preserve URL state.
 */
export const EntityTypesSearch = ({
  initialQuery = "",
  pageSize,
  sortBy,
  sortDir,
}: EntityTypesSearchProps) => {
  const hasActiveSearch = initialQuery.trim().length > 0;
  const clearHref = `/dashboard/entity-types?size=${pageSize}&sort=${sortBy}&dir=${sortDir}`;

  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
      <form
        action="/dashboard/entity-types"
        method="get"
        className="flex w-full max-w-sm flex-col gap-2 sm:flex-row sm:items-center"
        role="search"
        aria-label="Search entity types by name"
      >
        <input type="hidden" name="size" value={pageSize} />
        <input type="hidden" name="sort" value={sortBy} />
        <input type="hidden" name="dir" value={sortDir} />
        <div className="flex-1 space-y-2">
          <Label htmlFor="entity-types-search" className="sr-only">
            Search by name
          </Label>
