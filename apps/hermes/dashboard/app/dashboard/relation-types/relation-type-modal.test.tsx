...
vi.mock(
  "@/app/dashboard/relation-types/actions/create/.generated/use-form-action",
  () => ({
...
vi.mock(
  "@/app/dashboard/relation-types/actions/update/.generated/use-form-action",
  () => ({
    useFormAction: vi.fn(() => createMockUseFormAction()),
  }),
);
...
const getCreateUseFormActionMock = async () => {
  const mod =
    await import("@/app/dashboard/relation-types/actions/create/.generated/use-form-action");
  return mod.useFormAction as Mock;
};

describe("RelationTypeModal", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  ...
  it("in edit mode renders title and form for the row", () => {
    ...
    expect(screen.getByText("Edit relation type: CEO_OF")).toBeInTheDocument();
    expect(screen.getByLabelText("Name")).toHaveValue("CEO_OF");
    expect(screen.getByLabelText("Description")).toHaveValue("Reports to");
    expect(
