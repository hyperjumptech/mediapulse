...
/**
 * Creates the manual execution cancellation handler with dependency injection.
 *
 * @param deps - Optional cancellation collaborator for tests.
 * @returns Handler that cancels a manual execution idempotently.
 */
export const createCancelManualExecutionHandler = ({
  ...
  return async ({ body }) => {
    const result = await cancelExecution("manual", body.executionId);
...
/**
 * Handles manual execution cancellation requests.
 */
