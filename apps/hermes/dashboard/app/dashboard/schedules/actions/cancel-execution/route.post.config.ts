...
/**
 * Creates the schedule execution cancellation handler with dependency injection.
 *
 * @param deps - Optional cancellation collaborator for tests.
 * @returns Handler that cancels a schedule execution idempotently.
 */
export const createCancelScheduleExecutionHandler = ({
  ...
  return async ({ body }) => {
    const result = await cancelExecution("schedule", body.executionId);
...
/**
 * Handles schedule execution cancellation requests.
 */
