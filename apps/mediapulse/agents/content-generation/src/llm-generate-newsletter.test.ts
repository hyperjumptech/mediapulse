...
describe("generateNewsletterWithLlm — happy path", () => {
  ...
  it("passes an AbortSignal for the default 120000 ms timeout", async () => {
    // Setup
    const generateObjectFn = makeSuccessfulGenerateFn();
  ...
  it("passes an AbortSignal based on openai.timeoutMs", async () => {
    // Setup
    ...
    // Assert — signal is present and was created with the configured timeout
    const callArgs = (generateObjectFn as ReturnType<typeof vi.fn>).mock
      .calls[0]![0] as GenerateNewsletterObjectArgs;
    expect(callArgs.signal).toBeInstanceOf(AbortSignal);
  });
...
// ---------------------------------------------------------------------------
// Timeout → retryable → retry loop (fix #8 integration)
// ---------------------------------------------------------------------------

describe("generateNewsletterWithLlm — timeout treated as retryable", () => {
  it("retries when generateObject throws a TimeoutError", async () => {
    // Setup — TimeoutError is treated as retryable by isRetryableLlmError
    ...
    const sleepFn = vi.fn().mockResolvedValue(undefined);
    const generateObjectFn = vi.fn().mockRejectedValue(timeoutError);
  ...
  it("succeeds on second attempt after initial TimeoutError", async () => {
    // Setup
    ...
    const generateObjectFn = vi
      ...
      .mockResolvedValueOnce({
        object: {
          subject: "Recovered",
          executiveSummary: "After timeout.",
          topNews: [{ title: "News", summary: "Summary." }],
        },
      });
