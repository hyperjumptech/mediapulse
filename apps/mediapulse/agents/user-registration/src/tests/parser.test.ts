...
describe('Parser Helpers', () => {
  describe('normalizeEmail', () => {
    it('normalizes email addresses correctly', () => {
      ...
      expect(normalizeEmail('user.name+label@example.co.uk')).toBe('user.name+label@example.co.uk');
    });
  });
  ...
  describe('deriveNameFromEmailLocalPart', () => {
    it('derives names from standard local parts', () => {
      expect(deriveNameFromEmailLocalPart('john.doe@example.com')).toBe('John Doe');
      expect(deriveNameFromEmailLocalPart('jane_smith123@example.com')).toBe('Jane Smith123');
      expect(deriveNameFromEmailLocalPart('alice-bob@example.com')).toBe('Alice Bob');
  ...
  describe('extractSenderEmail', () => {
    ...
    it('returns null if no valid sender can be found', () => {
      ...
      expect(extractSenderEmail({ from: { emailAddress: { name: 'Only Name' } } })).toBeNull();
    });
  });
  ...
  describe('extractTickerSymbol', () => {
    ...
    it('returns null if neither subject nor body contains a valid ticker', () => {
      expect(extractTickerSymbol('Random email', 'Just saying hello')).toBeNull();
