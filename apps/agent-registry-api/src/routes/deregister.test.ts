...
describe("deregister", () => {
    ...
    it("should handle validation errors for invalid instanceId type", async () => {
        const validationErrorResponse = new Response(
            JSON.stringify({ message: "instanceId: Expected string, received number" }),
            { status: 400, headers: { "Content-Type": "application/json" } }
        );
        ...
        expect(response.status).toBe(400);
        expect(responseData.message).toBe("instanceId: Expected string, received number");
        expect(mockPrismaDeleteFn).not.toHaveBeenCalled();
    });
    ...
    it("should handle database error when instance not found", async () => {
        ...
        mockValidateBodyFn.mockResolvedValue(mockBody);
        mockPrismaDeleteFn.mockRejectedValue(new Error("Record to delete does not exist"));
    ...
    it("should handle generic database errors", async () => {
        ...
        mockValidateBodyFn.mockResolvedValue(mockBody);
        mockPrismaDeleteFn.mockRejectedValue(new Error("Database connection failed"));
